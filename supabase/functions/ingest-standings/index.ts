// Every 6h: fetch standings for current competitions from football-data.org
// and upsert. Missing FOOTBALL_DATA_API_KEY is a graceful skip.
...
Deno.serve(async () => {
  ...
  try {
    ...
    await logRun(db, footballDataAdapter.source, 'ingest-standings', result);
    return Response.json(result, { status: 200 });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    await logRun(db, footballDataAdapter.source, 'ingest-standings', {
      status: 'error',
      rowsUpserted: 0,
      error: message,
    });
